...
impl LanceDBService {
    ...
    /// Get or create a table for a model
    async fn get_table(&self, model_id: &str) -> AppResult<Table> {
    ...
    pub async fn store_embeddings(
        ...
        let table = self.get_table(model_id).await?;
    ...
    pub async fn get_stats(&self, model_id: &str) -> AppResult<ModelStats> {
        let table = match self.get_table(model_id).await {
            Ok(t) => t,
            Err(_) => {
