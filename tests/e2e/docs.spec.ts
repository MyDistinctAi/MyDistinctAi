...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto('http://localhost:4000/api/test-auth/johndoe')
...
test.describe('Documentation Site', () => {
  ...
  test('should navigate between sections', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'Features Guide' })).toBeVisible()
  ...
  test('should display self-hosting content', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'Self-Hosting Guide' })).toBeVisible()
