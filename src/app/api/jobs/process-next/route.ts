...
async function processFileJob(payload: any, supabase: any) {
  ...
  try {
    ...
    const { generateEmbeddings } = await import('@/lib/embeddings/ollama-embeddings')
    ...
    const embeddings = await generateEmbeddings(textChunks, (current, total) => {
      if (current % 5 === 0 || current === total) {
        console.log(`   Progress: ${current}/${total} embeddings generated`)
      }
    })
